...
(* Definition lower_grade (g : grade) : grade :=
  match g with
  | Grade l1 m2 => match m2 with
                | Plus => Grade l1 Natural
                | Natural => Grade l1 Minus
                | Minus => match l1 with
                          | _ => Grade (lower_letter l1) m2
                          end
                end
  end. *)
...
(* Definition lower_grade (g : grade) : grade :=
  match g with
  | Grade l1 m2 => match l1 with
                | _ => match m2 with
                      | Plus => Grade (lower_letter l1) Natural
                      | Natural => Grade (lower_letter l1) Minus
                      | Minus => Grade (lower_letter l1) Plus
                      end
                end
  end. *)
...
Definition lower_grade (g : grade) : grade :=
  ...
  | Grade l1 m2 => match l1, m2 with
                  | l1, Plus => Grade l1 Natural
                  | l1, Natural => Grade l1 Minus
                  | l1, Minus => Grade (lower_letter l1) Plus
                  end
  end.
...
Proof.
  ...
  destruct m.
    ...
    destruct l.
      ...
      simpl. reflexivity. 
      simpl. reflexivity. 
      intros x. rewrite <- x. simpl. reflexivity.
Qed.
...
Proof.
  intros l g H H1 H2. rewrite apply_late_policy_unfold.
  rewrite H. rewrite H1. reflexivity.
Qed. 
